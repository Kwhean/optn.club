...
import { BuildSettings, FH6Setup, TuneSettings } from './FH6Setup';
...
export function formatTune(form: FH6Setup, model: string): string[] {
  const text = [
...
function formatWheelUpgrades(upgrades: BuildSettings): string[] {
  const { front: fRim, rear: rRim } = upgrades.tiresAndRims.rimStyle;

  const wheels: Record<string, string> = {
    frontStyle: fRim.type,
    frontName: fRim.name,
    rearStyle: rRim.type,
    rearName: rRim.name,
    size: '',
...
function formatStatistics(form: FH6Setup, globalUnit: 'Metric' | 'Imperial') {
  const stats: string[][] = [];
...
function formatShareCodes(form: FH6Setup) {
  const codes: string[][] = [];
...
function formatHeader(form: FH6Setup) {
  const text: string[] = [];
...
export default function fh6DiscordGenerator(form: FH6Setup, globalUnit: GlobalUnit, linkUrl: string) {
  ...
  if (codes.length) {
    lines.push(bold('Share Codes'), '```', ...codes, '```');
  }
  ...
  lines.push('Formatted using:', 'https://optn.club/formatter/forza/horizon6/v1');
